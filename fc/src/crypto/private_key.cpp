...
namespace fc { namespace crypto {
   ...
   static private_key::storage_type parse_hexstr(const fc::sha256& shim)
   {
		return private_key::storage_type(sm2::private_key_shim(shim));
   }
   ...
   private_key::private_key(const fc::sha256& shim,const string& hex_str) // only test
   :_storage(storage_type(sm2::private_key_shim(shim)))  // this is only test
   {
   }
   ...
   std::string private_key::to_string(const fc::time_point& deadline) const
   {
	  ...
	  //std::cout<< " which storage " << which << std::endl;	
      if (which == 0) {
