...
namespace fc { namespace crypto { namespace sm2 {
    namespace detail
    {
    ...
    compact_signature signature_from_ecdsa(const EC_KEY* key, const public_key_data& pub_data, fc::ecdsa_sig& sig, const fc::sha256& d) {
        ...
        for (int i=0; i<4; i++)
        {
		  ...
		  keyRec.my->_key = EC_KEY_new_by_curve_name( NID_sm2 );
...
/*
    ...
    */
    ...
    public_key public_key::mult( const fc::sha256& digest )
    {
        ...
        rtn.my->_key = EC_KEY_new_by_curve_name( NID_sm2 );
    ...
    {
       private_key self;
	   self.my->_key = EC_KEY_new_by_curve_name( NID_sm2 );
    ...
    private_key private_key::generate()
    {
       ...
       EC_KEY* k = EC_KEY_new_by_curve_name( NID_sm2 );
       ...
       EC_KEY_free(k);
#endif
       fc::sha256 hash_key = self.get_secret();
       std::string secert_str = hash_key.str(); // test  by alvin
       std::cout<<"secert_str  " << secert_str << std::endl;
       return self;
    }
...
//        fprintf( stderr, "%d  %d\n", buf_len, sizeof(sha256) );
    ...
    bool       public_key::verify( const fc::sha256& digest, const fc::crypto::sm2::signature& sig )
    {
    ...
    {
	  serialize_ecc_point(); // test uncompressed by alvin
      public_key_data dat;
    ...
    public_key_point_data public_key::serialize_ecc_point()const
    {
      ...
      char* front = &dat.data[0];
      i2o_ECPublicKey( my->_key, (unsigned char**)&front  );
      std::string  pk_str = fc::to_hex( (char*)&dat.data[0], sizeof(public_key_point_data) ); // test 
       std::cout<<"public_uncompress_str  " << pk_str << std::endl;
      return dat;
    }
    ...
    public_key::public_key( const public_key_data& dat )
    {
      ...
      else
      {
         ...
         my->_key = EC_KEY_new_by_curve_name( NID_sm2 );
    ...
    bool       private_key::verify( const fc::sha256& digest, const fc::crypto::sm2::signature& sig )
    {
      return 1 == ECDSA_verify( 0, (unsigned char*)&digest, sizeof(digest), (unsigned char*)&sig, sizeof(sig), my->_key );
    }
    ...
    {
       public_key pub;
	   pub.my->_key = EC_KEY_new_by_curve_name( NID_sm2 );
    ...
    public_key::public_key( const compact_signature& c, const fc::sha256& digest, bool check_canonical )
    {
        ...
        BN_rshift1(halforder, order);
        if(BN_cmp(s, halforder) > 0)
           FC_THROW_EXCEPTION( exception, "invalid high s-value encountered in sm2 signature" );
...
//            fprintf( stderr, "compressed\n" );
  ...
  void to_variant( const crypto::sm2::private_key& var,  variant& vo )
  {
    vo = var.get_secret();
  }
  void from_variant( const variant& var,  crypto::sm2::private_key& vo )
  {
    fc::sha256 sec;
    from_variant( var, sec );
    vo = crypto::sm2::private_key::regenerate(sec);
  }

  void to_variant( const crypto::sm2::public_key& var,  variant& vo )
  {
    vo = var.serialize();
  }
  void from_variant( const variant& var,  crypto::sm2::public_key& vo )
  {
    crypto::sm2::public_key_data dat;
    from_variant( var, dat );
    vo = crypto::sm2::public_key(dat);
  }
