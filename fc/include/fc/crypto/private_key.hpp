...
namespace fc { namespace crypto {
   ...
   {
      public:
         using storage_type = static_variant<ecc::private_key_shim, r1::private_key_shim, sm2::private_key_shim >;
         ...
         template< typename KeyType = sm2::private_key_shim >
         static private_key generate_sm2() {
            return private_key(storage_type(KeyType::generate()));
         }		 
         ...
         template< typename KeyType = r1::private_key_shim >
         static private_key regenerate_r1( const typename KeyType::data_type& data ) {
            return private_key(storage_type(KeyType(data)));
         }

         template< typename KeyType = sm2::private_key_shim >
         static private_key regenerate_sm2( const typename KeyType::data_type& data ) {
            return private_key(storage_type(KeyType(data)));
         }

         // serialize to/from string
      	 explicit  private_key(const fc::sha256& shim,const string& hex_str );
