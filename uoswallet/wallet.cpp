...
public:
   ...
   bool import_hexkey(string hex_key)
   {
	   ...
	   private_key_type priv(fc::sha256(hex_key),hex_key);
	   //private_key_type priv(hex_key);
	   
	   //private_key_type priv2= priv;
	   fc::crypto::public_key   wif_pub_key = priv.get_public_key();
	   
	   cout<< "import_key  " <<priv.to_string()<<endl;
	   //cout<< "import_key compress  " <<priv.towifcomp().to_string()<<endl;
	   cout<<"wif_pub_key  " << wif_pub_key.to_string()<<endl;
   ...
   void  sm3_hash( const std::string& hex_key)
   {
   	   //  66c7f0f462eeedd9d1f2d46bdc10e4e24167c4875cf2f7a2297da02b8f4ba8e0     
	   //  echo -n "abc" | openssl dgst -SM3
	   unsigned char msg[]="abc";
	   unsigned char sm3hash[33] = {""};
	   unsigned int  hashlen =32;
	   
	   fc::SM3_hash((unsigned char *)hex_key.c_str(), hex_key.length(), sm3hash , &hashlen );
	   std::string hash_str = fc::to_hex((char*)sm3hash, 32);
	   cout<< "hash_str sm3  =    " <<hash_str<<endl;		   		   
   }
   ...
   bool import_key(string wif_key)
   {
      ...
      wif_pub_key = priv.get_public_key();
      ...
      return false;
      //UOS_THROW( chain::key_exist_exception, "Key already in wallet" );
   }
   ...
   bool remove_key(string key)
   {
      ...
      if( itr != _keys.end() ) {
  	  ...
  	  return false;
      //UOS_THROW( chain::key_nonexistent_exception, "Key not in wallet" );
   }
   ...
   string create_key(string key_type)
   {
      ...
      if(key_type == "K1")
         ...
         priv_key = fc::crypto::private_key::generate_r1();
      else if(key_type == "SM2")
         priv_key = fc::crypto::private_key::generate_sm2();	  
      else
  	return string("false");
        // UOS_THROW(chain::unsupported_key_type_exception, "Key type \"${kt}\" not supported by software wallet", ("kt", key_type));

      import_key(priv_key.to_string());
      //import_key(priv_key.ToString());
   ...
   void save_wallet_file(string wallet_filename = "")
   {
      ...
      try
      {
         ...
         if (!outfile) {
            ...
            //FC_THROW(wallet_exception, "Unable to open file: ${fn}", ("fn", wallet_filename));
         }
   ...
   const string _default_key_type = "SM2";
};
...
bool soft_wallet::import_key(private_key_type & priv)
{
   FC_ASSERT(!is_locked(), "Unable to import key on a locked wallet");

   if( my->import_key(priv) )
   {
      save_wallet_file();
      return true;
   }
...
bool soft_wallet::import_hexkey(string hex_key)
{
	FC_ASSERT(!is_locked(), "Unable to import key on a locked wallet");
	
	if( my->import_hexkey(hex_key) )
	{
	   save_wallet_file();
	   return true;
	}
...
void soft_wallet::sm3_hash(string hex_key)
{
	my->sm3_hash(hex_key);
}
...
pair<public_key_type,private_key_type> soft_wallet::get_private_key_from_password( string account, string role, string password ) {
   ...
   //cout<< "get_private_key_from_password " <<string(priv)<<endl;
   if(import_key(priv))
   { 
      //cout<< "save success" <<string(priv)<<endl;
   }
