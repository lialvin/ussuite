 ...
 */
#include <uosio/chain/block_log.hpp>
...
namespace uosio { namespace chain {
   ...
   void block_log::open(const fc::path& data_dir) {
      ...
      if (log_size) {
         ...
         UOS_ASSERT( version > 0, block_log_exception, "Block log was not setup properly with genesis information." );
         UOS_ASSERT( version == block_log::supported_version, block_log_unsupported_version,
   ...
   uint64_t block_log::append(const signed_block_ptr& b) {
      try {
         UOS_ASSERT( my->genesis_written_to_block_log, block_log_append_fail, "Cannot append to block log until the genesis is first written" );

         my->check_block_write();
         my->check_index_write();

         uint64_t pos = my->block_stream.tellp();
         UOS_ASSERT(my->index_stream.tellp() == sizeof(uint64_t) * (b->block_num() - 1),
   ...
   signed_block_ptr block_log::read_block_by_num(uint32_t block_num)const {
      try {
         ...
         if (pos != npos) {
            ...
            UOS_ASSERT(b->block_num() == block_num, reversible_blocks_exception,
   ...
   fc::path block_log::repair_log( const fc::path& data_dir, uint32_t truncate_at_block ) {
      ...
      UOS_ASSERT( fc::is_directory(data_dir) && fc::is_regular_file(data_dir / "blocks.log"), block_log_not_found,
      ...
      UOS_ASSERT( blocks_dir_name.generic_string() != ".", block_log_exception, "Invalid path to blocks directory" );
      backup_dir = backup_dir / blocks_dir_name.generic_string().append("-").append( now );

      UOS_ASSERT( !fc::exists(backup_dir), block_log_backup_dir_exist,
