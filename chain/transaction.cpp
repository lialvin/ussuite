...
void transaction_header::validate()const {
   UOS_ASSERT( max_net_usage_words.value < UINT32_MAX / 8UL, transaction_exception,
...
{ try {
   ...
   for(const signature_type& sig : signatures) {
      ...
      UOS_ASSERT( allow_duplicate_keys || successful_insertion, tx_duplicate_sig,
...
uint32_t packed_transaction::get_unprunable_size()const {
   ...
   UOS_ASSERT( size <= std::numeric_limits<uint32_t>::max(), tx_too_big, "packed_transaction is too big" );
...
struct read_limiter {
   ...
   {
      UOS_ASSERT(_total + count <= Limit, tx_decompression_error, "Exceeded maximum decompressed transaction size");
      _total += count;
...
bytes packed_transaction::get_raw_transaction() const
{
   try {
      switch(compression) {
         ...
         default:
            UOS_THROW(unknown_transaction_compression, "Unknown transaction compression algorithm");
      }
