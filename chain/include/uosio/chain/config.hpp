 ...
 */
#pragma once
//#include <uosio/chain/wasm_interface.hpp>
#include <fc/time.hpp>
#include <uosio/chain/name.hpp>
#include <uosio/chain/types.hpp>
#pragma GCC diagnostic ignored "-Wunused-variable"

namespace uosio { namespace chain { namespace config {
...
const static uint64_t system_account_name    = N(uosio);
const static uint64_t null_account_name      = N(uosio.null);
const static uint64_t producers_account_name = N(uosio.prods);
...
const static uint64_t uosio_auth_scope       = N(uosio.auth);
const static uint64_t uosio_all_scope        = N(uosio.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t uosio_any_name = N(uosio.any);
const static uint64_t uosio_code_name = N(uosio.code);
...
//const static uosio::chain::wasm_interface::vm_type default_wasm_runtime = uosio::chain::wasm_interface::vm_type::binaryen;
...
} } } // namespace uosio::chain::config

constexpr uint64_t UOS_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / uosio::chain::config::percent_100;
}

template<typename Number>
Number UOS_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + uosio::chain::config::percent_100 - uosio::chain::config::percent_1)  / uosio::chain::config::percent_100;
}
