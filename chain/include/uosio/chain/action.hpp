...
namespace uosio { namespace chain {
    ...
    *  In the context of uosio, every action is dispatched to the handler defined
   ...
   struct action {
      ...
      template<typename T>
      T data_as()const {
         UOS_ASSERT( account == T::get_account(), action_type_exception, "account is not consistent with action struct" );
         UOS_ASSERT( name == T::get_name(), action_type_exception, "action name is not consistent with action struct"  );
...
FC_REFLECT( uosio::chain::permission_level, (actor)(permission) )
FC_REFLECT( uosio::chain::action, (account)(name)(authorization)(data) )
