 ...
 */
#pragma once
#include <uosio/chain/abi_def.hpp>
#include <uosio/chain/trace.hpp>
...
namespace impl {
   ...
   struct abi_to_variant {
      ...
      {
         UOS_ASSERT( ++recursion_depth < abi_serializer::max_recursion_depth, abi_recursion_depth_exception, "recursive definition, max_recursion_depth ${r} ", ("r", abi_serializer::max_recursion_depth) );
   ...
   struct abi_from_variant {
                           ...
                           size_t recursion_depth, const fc::time_point& deadline, const fc::microseconds& max_serialization_time )
      {
         ...
         UOS_ASSERT( fc::time_point::now() < deadline, abi_serialization_deadline_exception, "serialization time limit ${t}us exceeded", ("t", max_serialization_time) );
         const variant_object& vo = v.get_object();
         UOS_ASSERT(vo.contains("account"), packed_transaction_type_exception, "Missing account");
         UOS_ASSERT(vo.contains("name"), packed_transaction_type_exception, "Missing name");
         ...
         UOS_ASSERT(valid_empty_data || !act.data.empty(), packed_transaction_type_exception,
                           ...
                           size_t recursion_depth, const fc::time_point& deadline, const fc::microseconds& max_serialization_time )
      {
         ...
         UOS_ASSERT( fc::time_point::now() < deadline, abi_serialization_deadline_exception, "serialization time limit ${t}us exceeded", ("t", max_serialization_time) );
         const variant_object& vo = v.get_object();
         UOS_ASSERT(vo.contains("signatures"), packed_transaction_type_exception, "Missing signatures");
         UOS_ASSERT(vo.contains("compression"), packed_transaction_type_exception, "Missing compression");
         ...
         if( vo.contains("packed_trx") && vo["packed_trx"].is_string() && !vo["packed_trx"].as_string().empty() ) {
            ...
            UOS_ASSERT(vo.contains("transaction"), packed_transaction_type_exception, "Missing transaction");
            transaction trx;
            vector<bytes> context_free_data;
