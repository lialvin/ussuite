...
#define N(X) uosio::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<uosio::chain::name> : private hash<uint64_t> {
      typedef uosio::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const uosio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, uosio::chain::name& check);
} // fc


FC_REFLECT( uosio::chain::name, (value) )
